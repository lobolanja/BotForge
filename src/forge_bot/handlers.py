...
def requiere_login(func):
    ...
    async def wrapper(update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if not verify_user(user_id):
            await update.message.reply_text("🔒 Access denied. Please log in using /login [password] to use this command.")
            return
...
async def translate(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"Translation to `{idioma_destino}`:\n{resultado}",parse_mode="Markdown")
...
async def status(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user:
        await update.message.reply_text(f"✅ You are logged in as: {user['username']}\n"
